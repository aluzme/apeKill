...
export default class Pricer {
	public static getOutGivenIn(currentReserve: Reserve, token0In: BigNumber = null, token1In: BigNumber = null, feeMultiplier: number = 9975): BigNumber {
		if (token0In.gt(0)) {
	...
	public static calcNewReserveExactIn(currentReserve: Reserve, token0In: BigNumber = null, token1In: BigNumber = null): Reserve {
		if (token0In.gt(0)) {
			const outWithFee = Pricer.getOutGivenIn(currentReserve, token0In, token1In);

			return new Reserve(currentReserve.reserve0.plus(token0In).toFixed(), currentReserve.reserve1.minus(outWithFee).toFixed());
		} else if (token1In.gt(0)) {
			const outWithFee = Pricer.getOutGivenIn(currentReserve, token0In, token1In);

			return new Reserve(currentReserve.reserve0.minus(outWithFee).toFixed(), currentReserve.reserve1.plus(token1In).toFixed());
		} else {
			throw new Error("One of them has to be non-zero");
		}
	}
